...
void ESPSerial::setup() {
    ...
    readThread = std::thread(&ESPSerial::readLoop, this);

    std::cout << "ESP connecté sur " << portName << std::endl;
}

void ESPSerial::readLoop() {
    std::string line;
    char c;
...
ESPSerial::ESPSerial(const std::string& port, int baudrate)
    : portName(port),
        baudRate(baudrate),
        serialPort(-1),
        running(false),
